...
std::vector<std::uint8_t> MsgpackSerializer::serialize(const Value& value) {
...
Value MsgpackSerializer::deserialize(const std::uint8_t* data, std::size_t size) {
    if (!data || size == 0) {
...
void MsgpackSerializer::packValue(const Value& v, msgpack::packer<msgpack::sbuffer>& pk) {
    using T = Value::Type;
